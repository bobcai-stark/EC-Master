...
public:
    CPcapFileBufferedWriter(EC_T_DWORD dwBufCnt, EC_T_CPUSET cpuAffinity /* EC_CPUSET_ZERO */, EC_T_DWORD dwPrio /* LOG_THREAD_PRIO */, EC_T_DWORD dwStackSize /* DEFAULT_LOG_STACK_SIZE */)
        : m_FrameBuffer(0 /* not used */, EC_NULL, (EC_T_CHAR*)"CPcapFileBufferedWriter"), m_cpuAffinity(cpuAffinity), m_dwPrio(dwPrio), m_dwStackSize(dwStackSize), m_dwFirstFrameTimeStampSecOffset(0)
    {
    ...
    {
        m_FrameBuffer.FreeInstance();
    }
    virtual EC_T_DWORD InitInstance(EC_T_DWORD dwBufCnt)
    {
        if (m_FrameBuffer.GetSize() > 0)
        {
            return EC_E_INVALIDSTATE;
        }
...
protected:
    ...
    EC_T_DWORD  m_dwFirstFrameTimeStampSecOffset;
#if (defined INCLUDE_PCAP_RECORDER_OS_PERF_MEAS)
...
public:
    ...
    EC_T_DWORD StartPcapRecorder(EC_T_DWORD dwInstanceId, EC_T_DWORD dwBufCnt, const EC_T_CHAR* szFileName = EC_NULL);
    EC_T_VOID  StopPcapRecorder();
