        ...
        while (pMsgBufferDesc->dwNextPrintMsgIndex != pMsgBufferDesc->dwNextEmptyMsgIndex)
        {
            ...
            if (pCurrMsg->dwMsgLen != 0)
            {
                ...
                if (pMsgBufferDesc->bSkipDuplicateMessages)
                {
                    ...
                    {
                        /* same message as before, just increment duplicate counter, ignoring empty duplicates */
                        if (!((pMsgBufferDesc->pszLastMsg[0] == '\0')
                          || ((pMsgBufferDesc->pszLastMsg[0] == '\n') && (pMsgBufferDesc->pszLastMsg[1] == '\0'))
                          || ((pMsgBufferDesc->pszLastMsg[0] == '\r') && (pMsgBufferDesc->pszLastMsg[1] == '\n') && (pMsgBufferDesc->pszLastMsg[2] == '\0'))))
                        {
...
EC_T_DWORD CPcapRecorder::StartPcapRecorder(EC_T_DWORD dwInstanceId, EC_T_DWORD dwBufCnt, const EC_T_CHAR* szFileName)
{
    EC_T_DWORD dwRes = EC_E_ERROR;
    EC_T_DWORD dwRetVal = EC_E_ERROR;
    ...
    CPcapFileBufferedWriter::FreeInstance();
...
EC_T_VOID CPcapRecorder::StopPcapRecorder()
{
    if (0xffff != m_dwInstanceId)
    {
        Uninstall();
...
EC_T_VOID CPcapFileBufferedWriter::FlushBuffer(EC_T_VOID)
{
    ...
    while (m_FrameBuffer.RemoveNoLock(Entry))
    {
        ...
        FrameHeader.TimeStamp.dwSec = (EC_T_DWORD)(Entry.Desc.qwTimestamp / 1000000000) + m_dwFirstFrameTimeStampSecOffset;
...
{
#if (defined EC_VERSION_LINUX)
    if (0 == m_dwFirstFrameTimeStampSecOffset)
    {
        struct timespec ts;
        clock_gettime(CLOCK_REALTIME, &ts);

        m_dwFirstFrameTimeStampSecOffset = (EC_T_DWORD)ts.tv_sec;
    }
#endif
