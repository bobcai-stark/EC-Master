...
#define MAX_OBNAME_LEN          ((EC_T_DWORD)100)
    ...
    for (wIndex = 0; (wIndex < wODListLen) && !*pbStopReading; wIndex++)
    {
        ...
        if (pAppContext->AppParms.dwAppLogLevel >= EC_LOG_LEVEL_INFO)
        {
            ...
            EC_T_WORD wObNameLen = EC_AT_MOST(pMbxGetObDescTfer->MbxData.CoE_ObDesc.wObNameLen, MAX_OBNAME_LEN);
            OsSafeStrcpy(szObName, pMbxGetObDescTfer->MbxData.CoE_ObDesc.pchObName, (EC_T_INT)wObNameLen + 1);
