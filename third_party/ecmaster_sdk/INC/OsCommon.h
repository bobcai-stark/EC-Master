...
#define EC_INSTRUMENT_MOCKABLE_INLINESTART virtual
#define EC_INSTRUMENT_MOCKABLE_INLINESTOP
#define EC_INSTRUMENT_MOCKABLE_FUNC virtual
#define EC_INSTRUMENT_MOCKABLE_VAR public:
#else
#define EC_INSTRUMENT_MOCKABLE_INLINESTART EC_INLINESTART
#define EC_INSTRUMENT_MOCKABLE_INLINESTOP
...
#endif
#ifndef EC_SETBITS
/** \brief 从源数据的第 1 位开始拷贝指定数量的 bit，到目标缓冲区指定的 bit 偏移位置
\note 本函数只应当用于设置“按位对齐（bit-aligned）”的数据；对于“按字节对齐（byte-aligned）”的数据，应使用对应的按字节函数。
\param pbyDstBuf   [out] 目标缓冲区：数据会被拷贝到这里
\param pbySrcData  [in]  源缓冲区：从第 1 位开始拷贝
\param nDstBitOffs [in]  目标缓冲区的 bit 偏移：从这里开始写入
\param nBitSize    [in]  要拷贝的 bit 数
*/
